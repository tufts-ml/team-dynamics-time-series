...
def plot_sample_with_system_regimes(
    x: NumpyArray2D,
    y: NumpyArray2D,
    z_entity: typing.List[int],
    z_system: typing.List[int],
) -> None:
    """
    Arguments:
        x: continuous states
    ...
    plt.subplot(411)
    plt.imshow(np.array(z_system)[None, :], aspect="auto", cmap=matplotlib.colormaps["cool"])
    plt.yticks([0], ["$z_{{\\mathrm{{true}}}}$"])
    plt.title("(System-Level) Regimes")

    plt.subplot(412)
    plt.imshow(np.array(z_entity)[None, :], aspect="auto", cmap=matplotlib.colormaps["hot"])
    plt.yticks([0], ["$z_{{\\mathrm{{true}}}}$"])
    plt.title("(Entity-Level) Regimes")
