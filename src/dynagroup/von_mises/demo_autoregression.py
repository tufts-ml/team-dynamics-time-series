...
# 2) drift angle is too big (like over -.5)

for drift_true in np.array([-0.5, -0.25, 0.0, 0.25, 0.5]) * np.pi:
    for ar_coef_true in [-0.9, -0.5, 0.0, 0.5, 0.9]:
        print("\n --- New test ---")
        ...
        assert np.isclose(params.drift, drift_true, atol=np.pi / 16)
