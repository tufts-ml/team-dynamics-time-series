...
from dynagroup.vi.M_step_and_ELBO import run_M_step_for_CSP_in_closed_form__Gaussian_case
...
def test_that_gaussian_initialization_diversely_assigns_entity_states_to_basketball_data(
    ...
    for j in range(DIMS.J):
        ...
        print(f"Counts of entity-level state assigments after bottom-level rARHMM initialization for player {j}: {counts_labels_post_bottom_level_ARHMM}")
        assert _l1_distance_of_counts_from_uniform(counts_labels_post_bottom_level_ARHMM) <= THRESHOLD_l1_distance_of_counts_from_uniform



@pytest.mark.skip(reason=f"This can be used to do a more detailed check than what is done by the more macroscopic test called "
